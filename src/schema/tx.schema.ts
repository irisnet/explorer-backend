...
TxSchema.statics.queryTxList = async function (query:ITxsQuery):Promise<IListStruct>{
    ...
    if ((query.beginTime && query.beginTime.length) || (query.endTime && query.endTime.length)) {queryParameters.time_unix = {};}
    if (query.beginTime && query.beginTime.length) { queryParameters.time_unix.$gte =  Number(query.beginTime) }
    if (query.endTime && query.endTime.length) { queryParameters.time_unix.$lte =  Number(query.endTime) }
...
TxSchema.statics.findAllServiceTx = async function (pageSize?:number):Promise<ITxStruct[]>{
    ...
    return await this.find(queryParameters).sort({time_unix:-1}).limit(Number(pageSize));;
};
...
TxSchema.statics.findBindServiceTxList = async function (
    ...
    if(pageNum && pageSize){
        ...
        return await this.find(queryParameters).sort({"time_unix": -1});
    }
